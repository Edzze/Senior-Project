...
def main():
    ...
    # Avoid redefining arguments added by other modules (e.g., video_dataset)
    if '--lr' not in parser._option_string_actions:
    ...
    # Choose a distributed backend that's compatible with the current build/platform.
    # NCCL may not be available on some Windows PyTorch builds; fall back to 'gloo' when needed.
    try:
        use_nccl = torch.cuda.is_available() and torch.distributed.is_nccl_available()
    except Exception:
        use_nccl = False
    ...
    # Determine local rank provided by torch.distributed launcher (LOCAL_RANK)
    # Fallback to dist.get_rank() when LOCAL_RANK is not set.
    local_rank = int(os.environ.get('LOCAL_RANK', os.environ.get('RANK', '0')))
    if torch.cuda.is_available():
        try:
            torch.cuda.set_device(local_rank)
            cuda_device_id = f'cuda:{local_rank}'
        except Exception:
    ...
    #print(model)
    if cuda_device_id.startswith('cuda'):
        model.cuda()
        # device id as integer for DDP
        dev_id = int(cuda_device_id.split(':')[-1])
